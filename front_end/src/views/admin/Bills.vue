...
const loadBills = async () => {
  loading.value = true;
  try {
    const { data } = await apiFetch("/api/admin/bills");
    if (data) {
      bills.value = data
        ...
        .map((bill) => ({
          ...
          amount: bill.total,
          items: bill.items.reduce((acc, item) => acc + item.amount, 0),
        }));
    }
  } catch (e) {
...
async function updateStatus(bill, status) {
  let endpoint = "";
  if (status === "COMPLETED") {
    endpoint = `/api/admin/bills/${bill.id}/complete`;
  } else if (status === "CANCELLED") {
    endpoint = `/api/admin/bills/${bill.id}/cancel`;
  } else {
    console.error("Unknown status update requested:", status);
    return false;
  }
