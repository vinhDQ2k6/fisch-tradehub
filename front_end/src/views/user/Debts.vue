  ...
  try {
    ...
    if (data) {
      debts.value = data.map((bill) => ({
        ...bill,
        itemCount: bill.items.reduce((acc, item) => acc + item.amount, 0),
      }));
    }
  } catch (e) {
...
const confirmPay = async () => {
  ...
  try {
    const { data } = await apiFetch(
      `/api/payments/create-payment-link/${selectedBill.value.id}`,
      {
        method: "POST",
      },
    );
...
const updateBillInList = (updatedBill) => {
  ...
  if (index !== -1) {
    debts.value[index] = {
      ...updatedBill,
      itemCount: updatedBill.items.reduce((acc, item) => acc + item.amount, 0),
    };
  }
};
</script>
      ...
      >
        ...
        <Column header="Actions" style="width: 12rem">
          <template #body="slotProps">
            <div class="flex gap-2">
              <Button
                v-if="slotProps.data.status === 'PENDING_PAYMENT'"
                icon="pi pi-wallet"
                label="Pay"
                size="small"
                severity="success"
        ...
        <template #empty>
          <div class="text-center p-8">
            <i class="pi pi-file text-surface-500 text-6xl mb-4"></i>
            <div class="text-xl font-semibold mb-2">No bills found</div>
        ...
        <template #expansion="slotProps">
          <div class="p-3">
            ...
            <DataTable :value="slotProps.data.items" class="p-datatable-sm">
              <Column field="fishName" header="Fish Name" sortable></Column>
              <Column field="amount" header="Quantity" sortable></Column>
